...
void SchemeAvx128::correctInputStatus(const OutputStream& maskOn, const OutputStream& maskOff, int id)
{
	...
	if (this->isNotMarkedToRecalculate())
	{
		...
		if ((_mm_movemask_epi8(_mm256_castsi256_si128(_mm256_cmpeq_epi64(sensDifference, _mm256_setzero_si256()))) & 0xFFFF) != 0xFFFF)
			this->markToFullRecalculating();
		else
		{
			const __m256i dynDifference = _mm256_and_si256(difference, this->dynSensitiveMask[0]);
			if ((_mm_movemask_epi8(_mm256_castsi256_si128(_mm256_cmpeq_epi64(dynDifference, _mm256_setzero_si256()))) & 0xFFFF) != 0xFFFF)
