...
{
	const size_t size = 16 * 6 * (this->nPrepareCircuits + this->nMainCircuits + this->nStaticSensitives + this->nDynamicSensitives);
...
void SchemeSse256::correctInputStatus(const OutputStream& maskOn, const OutputStream& maskOff, int id)
{
	...
	if (this->isNotMarkedToRecalculate() && (_mm_movemask_epi8(_mm_cmpeq_epi32(_mm_or_si128(_mm_xor_si128(oldStatus[0], this->status[0]), _mm_xor_si128(oldStatus[1], this->status[1])), _mm_setzero_si128())) != 0xFFFF))
		this->markToRecalculate();
}
